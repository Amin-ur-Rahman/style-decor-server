...
const runDB = async () => {
  try {
    ...
    serviceCentersColl = db.collection("ServiceCenters");
    bookingColl = db.collection("bookings");
    paymentColl = db.collection("paymentsHistory");
    decoratorColl = db.collection("decorators");
...
app.post("/users", async (req, res) => {
  try {
    ...
    if (searchDuplicate) return res.send({ message: "user already exists" });
...
app.get("/booking/:decoratorId/decorator", verifyFBToken, async (req, res) => {
  try {
...
app.post("/decorators", async (req, res) => {
  try {
    ...
    if (!postDecoResult.insertedId) {
      return res.status(400).send({ message: "bad request, operation failed" });
    }
...
app.get("/bookings/:email", verifyFBToken, async (req, res) => {
  try {
    ...
    const bookings = await bookingColl.find({ bookedByEmail: email }).toArray();
    if (bookings.length === 0) {
      return res.status(404).send({ message: "no data found" });
    }
    res.send(bookings);
...
app.patch("/on-payment-success", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send({ message: "Server error: payment update failed" });
  }
});
